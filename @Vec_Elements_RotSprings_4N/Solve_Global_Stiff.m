function [Kspr]=Solve_Global_Stiff(obj,node,U,Mspr,Cspr)
    ...
    % In the followin gpart the commented code are the non-vectorized
    % version of the calculation. The details can be found in the 
    % Liu and Paulino PRSA paper 
    ...
    % part2ii=-rkj_norm./(m_square.*m_square).*...
    % (m*(cross(rkj',m')')+cross(rkj,m).*m');
    ...
    % part2ll=rkj_norm./(n_square.*n_square).*...
    % (n*(cross(rkj',n')')+cross(rkj,n)*n');

    cross_rij_m=cross(rij',m')';
    part2ik_temp_1=m1.*rkj./m_square./rkj_norm...
        +(rkj_norm./m_square./m_square).*((m1.*cross_rij_m)...
        +(cross_rij_m(:,1).*m));
    part2ik_temp_2=m2.*rkj./m_square./rkj_norm...
        +(rkj_norm./m_square./m_square).*((m2.*cross_rij_m)...
        +(cross_rij_m(:,2).*m));
    part2ik_temp_3=m3.*rkj./m_square./rkj_norm...
        +(rkj_norm./m_square./m_square).*((m3.*cross_rij_m)...
        +(cross_rij_m(:,3).*m));

    part2ik_temp=[part2ik_temp_1,part2ik_temp_2,part2ik_temp_3];

    % part2ik=m*rkj'/(m_square*norm(rkj))+norm(rkj)/...
    % (m_square*m_square)*(m*(cross(rij,m))'+cross(rij,m)*m');

    cross_rkl_n=cross(rkl',n')';
    part2lj_temp_1=(n1.*rkj./n_square./rkj_norm)...
        -(rkj_norm./n_square./n_square).*((n1.*cross_rkl_n)...
        +(cross_rkl_n(:,1).*n));
    part2lj_temp_2=(n2.*rkj./n_square./rkj_norm)...
        -(rkj_norm./n_square./n_square).*((n2.*cross_rkl_n)...
        +(cross_rkl_n(:,2).*n));
    part2lj_temp_3=(n3.*rkj./n_square./rkj_norm)...
        -(rkj_norm./n_square./n_square).*((n3.*cross_rkl_n)...
        +(cross_rkl_n(:,3).*n));

    part2lj_temp=[part2lj_temp_1,part2lj_temp_2,part2lj_temp_3];

    % part2lj=n*rkj'/(n_square*norm(rkj))-norm(rkj)...
    % /(n_square*n_square)*(n*(cross(rkl,n))'+cross(rkl,n)*n');

    cross_rkj_rij_m=cross(rkj-rij,m);
    part2ij_temp_1=-m1.*rkj./(m_square.*rkj_norm)+rkj_norm./...
        (m_square.*m_square).*((m1.*cross_rkj_rij_m)+...
        (cross_rkj_rij_m(:,1).*m));
    part2ij_temp_2=-m2.*rkj./(m_square.*rkj_norm)+rkj_norm./...
        (m_square.*m_square).*((m2.*cross_rkj_rij_m)+...
        (cross_rkj_rij_m(:,2).*m));
    part2ij_temp_3=-m3.*rkj./(m_square.*rkj_norm)+rkj_norm./...
        (m_square.*m_square).*((m3.*cross_rkj_rij_m)+...
        (cross_rkj_rij_m(:,3).*m));

    part2ij_temp=[part2ij_temp_1,part2ij_temp_2,part2ij_temp_3];

    % part2ij=-m*rkj'/(m_square*norm(rkj))+norm(rkj)/...
    % (m_square*m_square)*(m*(cross(rkj-rij,m))'+cross(rkj-rij,m)*m');

    cross_rkj_rkl_n=cross(rkj-rkl,n);
    part2lk_temp_1=-(n1.*rkj./(n_square.*rkj_norm))-rkj_norm./...
        (n_square.*n_square).*((n1.*cross_rkj_rkl_n)+...
        (cross_rkj_rkl_n(:,1).*n));
    part2lk_temp_2=-(n2.*rkj./(n_square.*rkj_norm))-rkj_norm./...
        (n_square.*n_square).*((n2.*cross_rkj_rkl_n)+...
        (cross_rkj_rkl_n(:,2).*n));
    part2lk_temp_3=-(n3.*rkj./(n_square.*rkj_norm))-rkj_norm./...
        (n_square.*n_square).*((n3.*cross_rkj_rkl_n)+...
        (cross_rkj_rkl_n(:,3).*n));

    part2lk_temp=[part2lk_temp_1,part2lk_temp_2,part2lk_temp_3];

    %part2lk=-n*rkj'/(n_square*norm(rkj))-norm(rkj)/...
    % (n_square*n_square)*(n*(cross(rkj-rkl,n))'+cross(rkj-rkl,n)*n');
